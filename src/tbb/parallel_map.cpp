...
{
    std::vector<std::vector<int>> matrix(*matrix_size, std::vector<int>(*matrix_size, initial_value));
    return matrix;
}
...
#pragma region sequential
    ...
    std::cout << "Sequential result: " << validate_matrix(&c, &matrix_size) << " (" << elapsed_ms.count() << "ms)" << std::endl;
#pragma endregion
...
#pragma region parallel_map 1
    ...
    std::cout << "Parallel map 1 result: " << validate_matrix(&c, &matrix_size) << " (" << elapsed_ms.count() << "ms)" << std::endl;
#pragma endregion
...
#pragma region parallel_map 2
    ...
    tbb::parallel_for(
        ...
        [&c, &matrix_size, &a, &b](tbb::blocked_range<int> range) {
            for (int i = range.begin(); i != range.end(); i++)
            {
                tbb::parallel_for(
                    tbb::blocked_range<int>(0, matrix_size),
                    [&c, &matrix_size, &a, &b, &i](tbb::blocked_range<int> range) {
    ...
    elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    std::cout << "Parallel map 2 result: " << validate_matrix(&c, &matrix_size) << " (" << elapsed_ms.count() << "ms)" << std::endl;
#pragma endregion
