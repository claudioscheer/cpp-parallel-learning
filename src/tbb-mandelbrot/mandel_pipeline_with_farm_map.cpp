...
int main(int argc, char **argv)
{
    ...
    if (argc < 5)
    {
        printf("Usage: ./pipeline_with_farm_map [size of the image] [number of iterations] [number of retries] [number of threads]\n\n");
    }
    else
    {
        dim = atoi(argv[1]);
...
#if !defined(NO_DISPLAY)
    SetupXWindows(dim, dim, 1, NULL, "Mandelbrot pipeline(seq, farm(map(seq)), seq)");
#endif
