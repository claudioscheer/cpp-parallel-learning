...
class GenerateLine {
  public:
    Line *operator()(Line *line) const {
        ...
        tbb::parallel_for(
            tbb::blocked_range<int>(0, line->data->dim),
            [&line, &im](tbb::blocked_range<int> range) {
                for (int j = range.begin(); j != range.end(); j++) {
                    ...
                    line->M[j] =
                        (unsigned char)255 - ((k * 255 / line->data->niter));
                }
            });
        return line;
    }
...
int main(int argc, char **argv) {
    ...
    XInitThreads();
#endif
    printf("#pipeline(seq, farm(map(seq)), seq) pattern implementation!\n");
    ...
    if (argc < 5) {
        printf("Usage: ./pipeline_with_farm_map [size of the image] [number of "
...
#if !defined(NO_DISPLAY)
    SetupXWindows(dim, dim, 1, NULL,
                  "Mandelbrot pipeline(seq, farm(map(seq)), seq)");
#endif
    ...
    for (int r = 0; r < retries; r++) {
        ...
        tbb::parallel_pipeline(
            ...
            tbb::make_filter<void, Line *>(tbb::filter::serial, Input(data)) &
                tbb::make_filter<Line *, Line *>(tbb::filter::parallel,
                                                 GenerateLine()) &
